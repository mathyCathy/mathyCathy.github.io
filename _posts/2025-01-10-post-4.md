...
Below, it is demonstrated that a 2x2 factorial design with $N=4n$ subjects has the same precision for estimating the treatment effect of $A$ (and also of estimating the treatment effect of $B$) as a single RCT with $N=4n$ subjects evaluating treatment of $A$ only. 
...
Note that $\beta_A = 0.5\cdot[E(y|A=1,B=0)-E(y|A=0,B=0)+E(y|A=1,B=1)-E(y|A=0,B=1)]$. Each expectation can be estimated by the mean of the outcomes ($y$) within each treatment combination, i.e., a mean of $n$ values with resulting variance $\sigma^2/n$. 
...
* Normally distributed outcome modeled by $y = \widetilde{\beta_0} + \widetilde{\beta_A} I(A=1) + \epsilon$, where $\epsilon$ are i.i.d. $N(0,\sigma^2)$. 
